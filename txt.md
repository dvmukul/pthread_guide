...
If one thread signals the condition variable, other threads would probably want to wait for this signal. They may do so using one of two functions, **pthread_cond_wait()** or **pthread_cond_timedwait()**. Each of these functions takes a condition variable, and a mutex (which should be locked before calling the wait function), unlocks the mutex, and waits until the condition variable is signaled, suspending the thread's execution. If this signaling causes the thread to awake (see discussion of **pthread_cond_signal()** earlier), the mutex is automagically locked again by the wait function, and the wait function returns.
